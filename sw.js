...
self.addEventListener("activate", (event) => {
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(
        // 【修正】只刪除「屬於 Boss Tetris 自己、版本號不同」的舊快取：
        // 用嚴格的 prefix（boss-tetris-v 開頭）＋排除目前版本 兩個條件一起判斷，
        // 絕對不能只用「除了自己全部刪掉」，否則會誤刪同一個 origin 底下
        // 其他 App／其他 library 的 Cache Storage。
        // 完全不會碰 IndexedDB／localStorage 等使用者存檔資料，那些是不同的儲存機制。
        keys.filter((key) => key.startsWith("boss-tetris-v") && key !== CACHE_NAME)
...
self.addEventListener("fetch", (event) => {
  ...
  // 【修正】sw.js 自己絕對不能被這個 Service Worker 放進 Cache Storage。
  // 頁面端為了避免 iOS Safari 快取住舊的 sw.js 內容，會用 ./sw.js?_ts=xxx 這種
  // 每次都不同的 cache-busting URL 去抓「最新版本」；如果不排除，會造成
  // 每按一次「更新到最新版本」，Cache Storage 裡就多一筆 sw.js?_ts=... 的快取，
  // 而且完全不影響一般 App Shell（index.html / manifest.json）原本的快取邏輯。
